  ...
  return (
    <div className="mb-6 rounded-2xl border border-white/[0.07] bg-white/[0.02] overflow-hidden">
      ...
      <div className="px-6 py-5 space-y-4">
              ...
              <tbody>
                {sec.table.rows.map((row, ri) => (
                  <tr key={ri} className="border-b border-white/[0.04] hover:bg-white/[0.02]">
                    {row.map((cell, ci) => (
                      <td key={ci} className={`px-4 py-2.5 text-gray-300 ${ci === 0 ? "font-mono font-medium text-violet-300 whitespace-nowrap" : ""}`}>{cell}</td>
                    ))}
                  </tr>
                ))}
              </tbody>
        ...
        {sec.tip && (
          <div className="flex gap-3 rounded-xl border border-violet-500/20 bg-violet-500/[0.05] px-4 py-3">
            <span className="text-violet-400 shrink-0">💡</span>
            <p className="text-xs text-violet-300 leading-relaxed">{sec.tip}</p>
          </div>
        )}
        {sec.warning && (
...
export default function AdvancedLessonPage({ params }: Props) {
  ...
  const prev = allAdvancedLessons[idx - 1];
  const next = allAdvancedLessons[idx + 1];
    ...
    <div className="min-h-screen bg-[#050816] text-gray-200">
      <div className="max-w-4xl mx-auto px-6 py-10">
        ...
        <div className="flex items-center gap-2 text-sm text-gray-500 mb-6 flex-wrap">
          ...
          <span>/</span>
          <span className="text-gray-300 capitalize">{lesson.track.replace(/-/g, " ")}</span>
          <span>/</span>
          ...
          <ul className="space-y-2">
            {lesson.objectives.map((obj: string, i: number) => (
              <li key={i} className="flex gap-2 text-sm text-gray-300">
                <span className="text-violet-400 mt-0.5 shrink-0">✓</span>{obj}
              </li>
            ))}
          </ul>
        </Section>
          ...
          <Section title="📚 Prerequisites">
            <div className="flex flex-wrap gap-2">
              {lesson.prerequisites.map((p: string) => (
                <Link key={p} href={`/advanced/lessons/${p}`}
        ...
        {/* Concepts (fallback when no sections) */}
        {(!lesson.sections || lesson.sections.length === 0) && lesson.concepts?.length > 0 && (
        ...
        {lesson.trafficFlow && lesson.trafficFlow.length > 0 && (
          <Section title="🔄 Traffic Flow">
            <ol className="space-y-2">
              {lesson.trafficFlow.map((step: string, i: number) => (
        ...
        {lesson.yamlExamples && lesson.yamlExamples.length > 0 && (
          <Section title="📄 Config Examples">
            {lesson.yamlExamples.map((ex: { title: string; code: string; description: string }, i: number) => (
        ...
        <div className="mb-8 grid sm:grid-cols-2 gap-4">
          {lesson.labs?.length > 0 && (
            <Link href={`/advanced/lessons/${lesson.slug}/lab`}
              className="group flex items-center gap-4 p-5 rounded-2xl border border-violet-500/25 bg-violet-500/[0.04] hover:bg-violet-500/10 hover:border-violet-500/40 transition-all">
          ...
          {lesson.quiz?.length > 0 && (
            <Link href={`/advanced/lessons/${lesson.slug}/quiz`}
        ...
        {lesson.troubleshooting?.length > 0 && (
          <Section title="🔧 Troubleshooting">
            <div className="space-y-3">
              {lesson.troubleshooting.map((item: { symptom: string; possibleCause: string; check: string; fix: string }, i: number) => (
        ...
        {lesson.interviewQuestions?.length > 0 && (
          <Section title="💼 Interview Questions">
            <div className="space-y-3">
              {lesson.interviewQuestions.map((iq: { level: string; question: string; answerGuide: string }, i: number) => (
          ...
          <Section title="🎯 Portfolio Task">
            <div className="p-5 rounded-xl border border-violet-500/30 bg-violet-500/5">
              ...
              <ul className="space-y-1">
                {lesson.portfolioTask.deliverables.map((d: string, i: number) => (
        ...
        <div className="mt-10 grid grid-cols-2 gap-4">
          {prev ? (
            <Link href={`/advanced/lessons/${prev.slug}`}
              className="flex flex-col gap-1 rounded-xl border border-white/[0.07] hover:border-violet-500/20 bg-white/[0.02] hover:bg-white/[0.04] p-4 transition-all">
          ...
          {next ? (
            <Link href={`/advanced/lessons/${next.slug}`}
              className="flex flex-col gap-1 rounded-xl border border-white/[0.07] hover:border-violet-500/20 bg-white/[0.02] hover:bg-white/[0.04] p-4 transition-all text-right">
            ...
            <Link href="/advanced"
              className="flex flex-col gap-1 rounded-xl border border-violet-500/20 bg-violet-500/5 p-4 transition-all text-right">
              <span className="text-[10px] text-violet-600 uppercase tracking-wider">Done!</span>
              <span className="text-sm text-violet-300 font-medium">Back to Advanced →</span>
            </Link>
          )}
        </div>
