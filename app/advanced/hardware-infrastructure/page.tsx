...
export const metadata: Metadata = {
  title: "Network Hardware & Physical Infra | Advanced | NetPath Academy",
};
...
export default function HardwareInfrastructurePage() {
  const track = advancedTracks.find(t => t.id === "hardware-infrastructure");

  return (
    <div className="min-h-screen bg-[#050816] text-gray-200 p-6 md:p-10">
      <div className="max-w-4xl mx-auto">
        <div className="flex items-center gap-2 text-sm text-gray-500 mb-6">
          <Link href="/advanced" className="hover:text-amber-400 transition-colors">Advanced</Link>
          <span>/</span>
          <span className="text-gray-300">Network Hardware & Physical Infra</span>
        </div>

        <div className="mb-8">
          <h1 className="text-3xl font-bold text-white mb-2">🏗️ Network Hardware & Physical Infra</h1>
          <p className="text-gray-400">Cabling, Switch, Router, Data Center, AI/GPU Hardware</p>
          {track && (
        ...
        {track && track.lessons.length > 0 ? (
          <div className="grid gap-4">
            {track.lessons.map((lesson) => (
              ...
              >
                <div className="w-8 h-8 rounded-full bg-gray-700/60 flex items-center justify-center text-sm font-bold text-gray-400 group-hover:bg-amber-500/20 group-hover:text-amber-400 shrink-0">
                  {lesson.order}
                </div>
                <div className="flex-1 min-w-0">
                  <h2 className="text-base font-semibold text-white mb-1 group-hover:text-amber-100">{lesson.title}</h2>
                ...
                <span className="text-gray-600 group-hover:text-amber-400 text-lg mt-1">→</span>
              </Link>
            ))}
          </div>
        ) : (
        ...
        <div className="mt-8 flex gap-4">
          <Link href="/advanced" className="text-sm text-gray-500 hover:text-amber-400 transition-colors">
            ← กลับ Advanced
          </Link>
