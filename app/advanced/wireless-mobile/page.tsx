...
export const metadata: Metadata = {
  title: "Wireless & Mobile | Advanced | NetPath Academy",
};
...
export default function WirelessMobilePage() {
  const track = advancedTracks.find(t => t.id === "wireless-mobile");
    ...
    <div className="min-h-screen bg-[#050816] text-gray-200 p-6 md:p-10">
      <div className="max-w-4xl mx-auto">
        ...
        <div className="flex items-center gap-2 text-sm text-gray-500 mb-6">
          ...
          <span>/</span>
          <span className="text-gray-300">Wireless & Mobile</span>
        </div>

        <div className="mb-8">
          <h1 className="text-3xl font-bold text-white mb-2">📡 Wireless & Mobile</h1>
          <p className="text-gray-400">Wi-Fi 7, Private 5G, O-RAN, Network Slicing</p>
          {track && (
          ...
          <div className="grid gap-4">
            {track.lessons.map((lesson) => (
              <Link
                ...
                className="flex items-start gap-4 p-5 rounded-xl border border-gray-700/40 bg-gray-800/20 hover:border-emerald-500/30 hover:bg-emerald-500/5 transition-all group"
              >
                <div className="w-8 h-8 rounded-full bg-gray-700/60 flex items-center justify-center text-sm font-bold text-gray-400 group-hover:bg-emerald-500/20 group-hover:text-emerald-400 shrink-0">
                  {lesson.order}
                </div>
                <div className="flex-1 min-w-0">
                  <h2 className="text-base font-semibold text-white mb-1 group-hover:text-emerald-100">{lesson.title}</h2>
                ...
                <span className="text-gray-600 group-hover:text-emerald-400 text-lg mt-1">→</span>
              </Link>
            ))}
          </div>
        ) : (
        ...
        <div className="mt-8 flex gap-4">
          <Link href="/advanced" className="text-sm text-gray-500 hover:text-emerald-400 transition-colors">
            ← กลับ Advanced
          </Link>
