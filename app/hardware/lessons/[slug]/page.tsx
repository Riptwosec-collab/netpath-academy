        ...
        {lesson.labs.length > 0 && (
          <Section title="🧪 Labs">
            {lesson.labs.map((lab, i) => (
              <div key={i} className="mb-4 p-4 rounded-xl border border-amber-500/20 bg-amber-500/5">
                <div className="flex items-center justify-between mb-3">
                  <h3 className="text-sm font-semibold text-amber-400">{lab.title}</h3>
                ...
                <ol className="space-y-1.5">
                  {lab.steps.map((step, si) => (
                    <li key={si} className="flex gap-2 text-xs text-gray-300">
                      <span className="text-amber-400 shrink-0 font-bold">{si+1}.</span>{step}
                    </li>
                  ))}
                </ol>
              </div>
